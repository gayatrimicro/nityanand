...
/* Media.Australia.com SCode 20081023 V1 */
...
var s_account="tamedia";
var s=s_gi(s_account);
s.visitorNamespace="tourismaustralia";
s.dc=122;
s.trackingServer="omniture.australia.com";
...
s.linkInternalFilters="javascript:,media.australia.com";
...
s.Media.trackEvets="None";
*/

s.siteID="";
s.defaultPage="default.aspx";
s.queryVarsList="cat,xcid,region";
s.pathExcludeDelim=";";
s.pathConcatDelim=":";
s.pathExcludeList="";
...
/*********************** TRAFFIC VARIABLES **************************/
	...
	if (!s.pageName) {
		...
		s.pageName=replaceAll(s.pageName,'_','-');
		s.pageName=replaceAll(s.pageName,'.aspx','');
	}
	
	if (!s.channel) {
		s.channel=s.split(s.pageName,s.pathConcatDelim);
		s.channel=s.channel[1];
		s.channel="media.australia.com:" + s.channel;
		s.channel=replaceAll(s.channel,'.aspx','');
	}

	if ((s.pageName.indexOf == ':multimediaresources') || (s.pageName.indexOf == ':tourismresearch')  || (s.pageName.indexOf == ':australianmediakit')) {
		s.channel="media.australia.com:usefullinks";
	}

	s.prop1=s.getQueryParam('keyword').toLowerCase();
	s.prop2=replaceAll(s.prop1,' ',',');

	if ((s.prop2 == '') && (s.pageName == 'search.aspx'))
		s.prop2="search results";
	if (s.prop2 == '')
		s.prop2="other";

	s.prop4=document.title.toLowerCase();

	/* if (!s,prop5) {
		s.prop5=s.split(s.pageName,s.pathConcatDelim);
		s.prop5=s.prop5[1];
	}*/

	s.hier1=replaceAll(s.hier1,':','|');
	if (s.hier1 == '')
		s.hier1="other";

/*********************** COMMERCE VARIABLES *************************/

	s.campaign=s.getQueryParam('cid,ta_cid').toLowerCase();
	s.campaign=s.getValOnce(s.campaign,'tam_s_campaign',0);

	s.eVar1=s.getQueryParam('intcmp,ta_intcmp').toLowerCase();
	s.eVar1=s.getValOnce(s.eVar1,'tam_s_evar1',0);
	...
	if ((s.pageName.indexOf == 'subscriptions/thankyou.aspx') && (document.referrer.indexOf('subscriptions/default.aspx') >= 0))
		s.events+=",event2";

	if (s.pageName.indexOf == '-rss.aspx') // RSS FEED
		s.events+=",event3";
...
/* Utility Function: Split a string (compatible with Javascript 1) */
...
/* Utility Function: Determine If Value Exists Within An Array */
...
/* getQueryParam 2.1 - return query string parameter(s) */
